...
scalar omegaKnoppWallFunctionFvPatchScalarField::yPlusLam
(
    const scalar kappa,
    const scalar E
)
...
omegaKnoppWallFunctionFvPatchScalarField::omegaKnoppWallFunctionFvPatchScalarField
(
    const omegaKnoppWallFunctionFvPatchScalarField& kwfpsf
)
:
...
void omegaKnoppWallFunctionFvPatchScalarField::updateCoeffs()
{
    ...
    const scalar cmu5 = pow(0.09, 0.5);
    ...
    forAll(omegaw, facei)
    {
        ...
        scalar ds = 0.03 * Ks_ * min( 1., pow(KsPlus/30., 0.6666666666))
                                  * min( 1., pow(KsPlus/45., 0.25))
                                  * min( 1., pow(KsPlus/60., 0.25));

        omegaw[facei] = min(
            ut / ( cmu5 * kappa_ )/ds,
            60.*nuw[facei]/0.075/sqr(y[facei])
            );
    }
...
void omegaKnoppWallFunctionFvPatchScalarField::evaluate
(
